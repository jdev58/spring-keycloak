...
public class KeycloakInitializer implements InitializingBean {
    ...
    public void initKeycloakUser(KeycloakUser user) {

        List<UserRepresentation> lst =  keycloak.realm(REALM_ID).users().searchByAttributes("username:ali2");
        ...
        List<RoleRepresentation> rolesToAdd=
                Arrays.asList(keycloak.realm(REALM_ID).roles().get("ROLE_USER").toRepresentation());
        userResource.roles().realmLevel().add(rolesToAdd);

        if (user.isAdmin()) {
            userRepresentation =
                    keycloak.realm(REALM_ID).users().search(user.getUsername()).get(0);
            UserResource userResourceAdmin =
                    keycloak.realm(REALM_ID).users().get(userRepresentation.getId());
            List<RoleRepresentation> rolesToAdd2=
                    Arrays.asList(keycloak.realm(REALM_ID).roles().get("admin").toRepresentation());
            userResource.roles().realmLevel().add(rolesToAdd2);
        }
    }
